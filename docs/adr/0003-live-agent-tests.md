...
other scenario: their steps drive headless host sessions (`claude -p`,
...
- Deleting the un-stageable "host cannot be determined" scenario — rejected: a
  new host may make it stageable; it stays as recorded `@todo` debt instead.

## Consequences

- The live suite needs both host CLIs installed and logged in; a missing CLI
  ...
  it). The harness strips inherited host-app env vars, so it can run even from
