# language: zh-CN
功能: BDD 规则同步
  BDD.md 是规则的唯一事实来源。插件会在 coding agent 的规范指令文件中维护
  ...
  场景: 安装会为选定 coding agent 放置技能
    假如 正在为某个 coding agent 安装 gherkin-bdd 的项目
    当 技能已安装
    那么 技能、BDD 规则和技能脚本被放入 coding agent 的项目技能目录
    而且 项目中没有新增其他内容

  场景: 安装会运行同一个同步逻辑
    假如 正在为某个 coding agent 安装 gherkin-bdd 的项目
    当 技能已安装
    那么 coding agent 的规范指令文件引用 BDD.md
