...
All user-facing functionality in this project must be described by a Gherkin file.

Gherkin files use the `.feature` extension and are the source of truth for application behavior. Product requirements, implementation plans, UI changes, test code, and agent instructions must align with the relevant `.feature` files.
...
- Treat Gherkin keywords and step wording as product language. In Chinese Gherkin files, use the official `# language: zh-CN` header and localized keywords such as `功能`, `场景`, `假如`, `当`, and `那么`.
