...
**The install contract is two skills + one hook.** `install_skill` copies exactly `SKILL.md`, `BDD.md`, and `scripts/check_bdd_sync.py` into `<skills-dir>/gherkin-bdd/`; `install_bootstrap_skill` copies `SKILL.md` into `<skills-dir>/bdd-bootstrap/` (the in-session skill that re-runs the installer for the detected host); nothing else ships. `HOST_SKILL_DIRS` / `HOST_HOOK_FILES` in [bin/bdd-bootstrap](bin/bdd-bootstrap) map each host to its skills directory (Claude → `.claude/skills`, Codex → `.agents/skills`) and hook config (Claude → `.claude/settings.json`, Codex → `.codex/hooks.json`). Adding a new shipped file means adding an explicit `copy_path` call.
